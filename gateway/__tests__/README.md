# AI Services Gateway Tests

This directory contains integration and end-to-end tests for the AI Services Gateway.

## Test Structure

- **integration/**: Integration tests that test the gateway in isolation without requiring running services
- **e2e/**: End-to-end tests that require the full AI services stack to be running

## Requirements Tested

### Integration Tests (gateway.integration.test.ts)
- **Requirement 1.7**: Gateway endpoint exposure
- **Requirement 4.1**: API key authentication with valid and invalid keys
- **Requirement 3.6**: Health check aggregation
- **Requirement 14.34**: Trace ID propagation
- **Requirement 16.1**: Response headers

### E2E Tests (gateway.e2e.test.ts)
- **Requirement 1.7**: End-to-end request flow through gateway to services
- **Requirement 4.1**: Authentication with valid and invalid keys
- **Requirement 3.6**: Health check aggregation with real services
...
describeE2E('End-to-End Request Flow', () => {
  it('should successfully process LLM request', async () => {
    const response = await request(baseURL)
      .post('/v1/llm/generate')
      .set('Authorization', `Bearer ${validApiKey}`)
      .send({ prompt: 'Hello', max_tokens: 50 });
...
2. Check gateway logs: `docker-compose logs ai-gateway`
...
3. Check service logs: `docker-compose logs llm-service rag-service sentiment-service`
...
- [AI Services API Documentation](../../API_DOCUMENTATION.md)
