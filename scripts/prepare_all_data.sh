#!/bin/bash
# Prepare ALL data for 4 services (no sampling)
# RAG + LLM + Sentiment + Ranker
...
# 1. RAG Service - Extract ALL 1000 contexts
echo "1️⃣ RAG Service: Extracting ALL contexts from conversations_1000.jsonl"
python scripts/data-processing/extract_contexts_for_rag.py \
  --input data/processed/llm/conversations_1000.jsonl \
  --output ../socialcue-space/rag/contexts_full.json

echo ""
echo "✅ RAG: contexts_full.json created with ALL 1000 contexts"
echo ""

# 2. LLM Service - Already has conversations_1000.jsonl
...
# 3. Sentiment Service - Already has sentiment_crush_dataset_500.jsonl
...
# 4. Ranker Service - Already has ranking_labels_1000.jsonl + trained model
...
echo "   2. Copy to Space: cp -r rag_index ../socialcue-space/rag/chroma_db"
echo "   3. Copy ranker: cp -r models/ranker_pairwise/* ../socialcue-space/models/ranker/"
